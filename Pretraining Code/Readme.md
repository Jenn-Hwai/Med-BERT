...
- <vocab>: is the path of a vocabulary (types) file which includes a dictionary that maps different ICD9/10 codes to integer values.(詞彙表文件路徑。如果是第一次運行且還沒有詞彙表，可以填入 NA，腳本會根據輸入資料自動生成。)   
